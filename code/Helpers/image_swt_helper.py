...
from teste import refined_image
Image = np.ndarray
...
def main():
    ...
    is_bright_on_dark = False

    # Open the image and obtain a grayscale representation.
    im = open_grayscale(image_local)  # TODO: Magic numbers hidden in arguments
    im = refined_image(image_local)
    cv2.imshow('refina', im)
    ...
    swt = apply_swt(im, edges, gradients, not is_bright_on_dark)
